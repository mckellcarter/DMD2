"""
FID evaluation for multi-step EDM models.

This script evaluates checkpoints with multi-step sampling and optional CFG.
"""

from third_party.edm.training.networks import EDMPrecond
from main.edm.edm_network import get_imagenet_edm_config
from main.edm.sample_edm_multistep import sample_multistep_cfg, get_sigmas_karras
...
def get_imagenet_config(label_dropout=0):
    base_config = {
        "img_resolution": 64,
        "img_channels": 3,
        "label_dim": 1000,
...
def create_generator(checkpoint_path, base_model=None, label_dropout=0):
    if base_model is None:
        base_config = get_imagenet_config(label_dropout=label_dropout)
...
def sample(accelerator, current_model, args, model_index):
    ...
    while len(all_images_tensor) * args.eval_batch_size * accelerator.num_processes < args.total_eval_samples:
        ...
        # Multi-step sampling with CFG
        eval_images = sample_multistep_cfg(
            generator=current_model,
            noise=noise,
            labels=one_hot_labels,
            num_steps=args.num_denoising_step,
    ...
    if accelerator.is_main_process:
        ...
        data_dict = {
            ...
            "num_denoising_steps": args.num_denoising_step,
            "guidance_scale": args.guidance_scale
        }
...
def evaluate():
    ...
    # Load previous stats
    info_path = os.path.join(folder, "stats_multistep.json")
    ...
    if accelerator.is_main_process:
        ...
        print(f"Using {args.num_denoising_step} denoising steps with CFG scale {args.guidance_scale}")

    # Initialize wandb
    if accelerator.is_main_process:
        run = wandb.init(
            config=args, dir=args.folder,
    ...
    while True:
        ...
        for checkpoint in new_checkpoints:
            ...
            if accelerator.is_main_process:
                ...
                stats["fid"] = fid
                stats["num_steps"] = args.num_denoising_step
                stats["guidance_scale"] = args.guidance_scale
        ...
        if accelerator.is_main_process:
            with open(os.path.join(folder, "stats_multistep.json"), "w") as f:

...
class EDMImageNetAdapter(HookMixin, GeneratorAdapter):
    ...
    def from_pickle(
        ...
        """
        Load from original EDM pickle format.

        This is the preferred loading method for official EDM checkpoints
        like `edm-imagenet-64x64-cond-adm.pkl`.

        Args:
            pickle_path: Path or URL to .pkl file
            device: Target device
        """
        # Import dnnlib from third_party/edm
        import sys
        from pathlib import Path

        # Ensure third_party/edm is in path for dnnlib
        edm_path = Path(__file__).parents[2] / 'third_party' / 'edm'
        if str(edm_path) not in sys.path:
            sys.path.insert(0, str(edm_path))

        import dnnlib

        print(f"Loading EDM from {pickle_path}...")
    ...
    def from_checkpoint(
        ...
        **kwargs
        ...
        Automatically detects pickle vs PyTorch format based on extension.

        Args:
            checkpoint_path: Path to model weights (.pkl, .pth, .bin)
            device: Target device
            label_dropout: Label dropout (ignored for pickle format)
        """
        if checkpoint_path.endswith('.pkl') or '://' in checkpoint_path:
            return cls.from_pickle(checkpoint_path, device)

        # PyTorch format - create fresh model and load weights
        from third_party.edm.training.networks import EDMPrecond
        from main.edm.edm_network import get_imagenet_edm_config
        from visualizer.model_utils import load_checkpoint

        config = cls.get_default_config()
        config.update(get_imagenet_edm_config(label_dropout=label_dropout))
